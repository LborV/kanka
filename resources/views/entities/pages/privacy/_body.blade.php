        ...
        <x-forms.field field="visibility" :label="__('entities/permissions.toggle.label')">
            <div class="flex flex-col gap-2">
                ...
                <div class="rounded-xl border border-base-300 p-2 flex gap-2 items-start cursor-pointer hover:shadow-sm">
                    ...
                    <i class="fa-solid-check-double text-success" x-show="status === false" x-cloak aria-label="Saved" data-title="{{ __('entities/permissions.quick.success.public', ['entity' => $entity->name]) }}" data-toggle="tooltip"></i>
                </div>
            </div>
        </x-forms.field>
        ...
        @if (!empty($visibility['roles']) || !empty($visibility['users']))
            <div class="flex flex-wrap gap-2 items-center " :class="{ 'line-through text-neutral-content': private === '1'}">
                @foreach ($visibility['roles'] as $role)
                    <span>
                        <x-icon class="fa-solid-user-group" />
                        @can('update', $role)
                        ...
                        @if ($role->isPublic() && $campaign->isPrivate())
                            <x-icon class="fa-solid-exclamation-triangle text-accent" tooltip :title="__('campaigns.roles.permissions.helpers.not_public')" />
                        @endif
                    </span>
                @endforeach
