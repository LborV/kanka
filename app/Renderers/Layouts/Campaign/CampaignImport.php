            ...
            'status' => [
                ...
                'render' => function (\App\Models\CampaignImport $model) {
                    if ($model->status_id === CampaignImportStatus::FAILED) {
                        return '<span class="text-error"><i class="fa-solid fa-xmark-circle" aria-hidden="true"></i> ' . __('campaigns/import.status.failed') . '</span>';
                    } elseif ($model->status_id == CampaignImportStatus::QUEUED) {
                        return '<span class="text-neutral-content"><i class="fa-solid-hourglass" aria-hidden="true"></i> ' . __('campaigns/import.status.queued') . '</span>';
                    } elseif ($model->status_id == CampaignImportStatus::FINISHED) {
                        return '<span class="text-success"><i class="fa-solid-check-circle" aria-hidden="true"></i> ' . __('campaigns/import.status.finished') . '</span>';
                    }

                    return '<span class="text-neutral-content"><i class="fa-solid-spinner fa-spin" aria-hidden="true"></i> ' . __('campaigns/import.status.running') . '</span>';
                },
            ],
        ];
